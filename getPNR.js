  ...
  try {
    ...
    const pnrDetails = await page.evaluate(() => {
      ...
      passengerRows.forEach((row, index) => {
        ...
        detailsText += `Seat/Berth: ${row.querySelector("td:nth-child(1) span")?.textContent?.trim() || 'N/A'}\n`;
        detailsText += `Current Status: ${row.querySelector("td:nth-child(2) span:first-child")?.textContent?.trim() || 'N/A'}\n`;
        detailsText += `Prediction: ${row.querySelector("td:nth-child(2) span:last-child")?.textContent?.trim() || 'N/A'}\n`;
        detailsText += `Booking Status: ${row.querySelector("td:nth-child(3) span")?.textContent?.trim() || 'N/A'}\n`;
        detailsText += `Coach Position: ${row.querySelector("td:nth-child(4) span")?.textContent?.trim() || 'N/A'}\n\n`;
      });
